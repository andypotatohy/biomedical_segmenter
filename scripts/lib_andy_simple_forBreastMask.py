...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr  2 20:08:03 2020
...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 21 19:20:14 2020

@author: andy adapted from lukas
"""
...
#    high = (c_int*3)(D1+(dpatch[0]/2)+dpatch[0]%2,D2+(dpatch[1]/2)+dpatch[1]%2,D3+(dpatch[2]/2)+dpatch[2]%2)
    low = (c_int*3)(D1-(dpatch[0]/2),D2-(dpatch[1]/2),D3-(dpatch[2]/2))
...
#                    z = random.choice(xrange(dpatch[2]/2,int(target_shape[2])-(dpatch[2]/2)+1))
                for _ in range(patches_per_subject/2):
                    x = random.choice(xrange(dpatch[0]/2,int(target_shape[0])-(dpatch[0]/2))) 
                    y = random.choice(xrange(dpatch[1]/2,int(target_shape[1])-(dpatch[1]/2)))
                    z = random.choice(xrange(dpatch[2]/2,int(target_shape[2])-(dpatch[2]/2)))
                    scanVoxels.append([x,y,z])      
            else:
    ...
    if np.sum(label_data) == 0:
      ...
      for j in range(len(voxelCoordinates)):
        ...
        labels.append(label_data[D1-(output_dpatch[0]/2):D1+(output_dpatch[0]/2)+output_dpatch[0]%2,
                                 D2-(output_dpatch[1]/2):D2+(output_dpatch[1]/2)+output_dpatch[1]%2,
                                 D3-(output_dpatch[2]/2):D3+(output_dpatch[2]/2)+output_dpatch[2]%2])
#        labels.append(proxy_label.dataobj[D1-output_dpatch[0]/2:D1+(output_dpatch[0]/2)+output_dpatch[0]%2,
#                                          D2-output_dpatch[1]/2:D2+(output_dpatch[1]/2)+output_dpatch[1]%2,
#                                          D3-output_dpatch[2]/2:D3+(output_dpatch[2]/2)+output_dpatch[2]%2])
    proxy_label.uncache()
...
#    for j in range(len(voxelCoordinates)):
#        D1,D2,D3 = voxelCoordinates[j]
#        labels.append(myReadNiftiPatch(subject,D1,D2,D3,output_dpatch))
    # del label_data
    return labels
...
#        if np.array(label_data.shape != shapes).any():
#            label_data = resize(label_data, shapes[i], order=0, preserve_range=True, anti_aliasing=True)        
        #DEBUG
        #out = nib.Nifti1Image(label_data, np.diag([1,1,1,0]))
        #nib.save(out, '/home/deeperthought/Projects/MultiPriors_MSKCC/DEBUG/' + 'Label_' + subject.split('MSKCC')[-1].replace('/','_label_'))
        
        padding_border = np.max(output_dpatch)
        label_padded = np.pad(label_data,((0,padding_border),(0,padding_border),(0,padding_border)),'constant')  # need to pad for segmentation with huge patches that go outside (only the end - ascending coordinates) boundaries. Scale stays the same, as the origin is not modified. 
        if np.sum(label_data) == 0:
...
#    bV = np.array(subject_channel_voxelCoordinates)
#    
#    if (len(range(np.min(bV[:,0])-(dpatch[0]/2),np.min(bV[:,0])+(dpatch[0]/2)+dpatch[0]%2)) != dpatch[0]) or \
#       (len(range(np.max(bV[:,0])-(dpatch[0]/2),np.max(bV[:,0])+(dpatch[0]/2)+dpatch[0]%2)) != dpatch[0]) or \
#       (len(range(np.min(bV[:,1])-(dpatch[1]/2),np.min(bV[:,1])+(dpatch[1]/2)+dpatch[1]%2)) != dpatch[1]) or \
...
#    # PERMUTE ON THE FLY # ANDY 2020-02-16
    ...
    for j in range(n_patches):      
        ...
        if not fullSegmentationPhase:
            ...
            try:
#                vol[j,:,:,:] = myReadNiftiPatch(subject,D1,D2,D3,dpatch)
                ...
                vol[j,:,:,:] = img_data[D1-(dpatch[0]/2):D1+(dpatch[0]/2)+dpatch[0]%2,
                                        D2-(dpatch[1]/2):D2+(dpatch[1]/2)+dpatch[1]%2,
                                        D3-(dpatch[2]/2):D3+(dpatch[2]/2)+dpatch[2]%2]
##                 !!!!!! ONLY LOAD THE NEEDED IMAGE PATCHES !!!!! ANDY 2020-03-11
...
#        if (len(range(D1-(dpatch[0]/2),D1+(dpatch[0]/2)+dpatch[0]%2)) != dpatch[0]) or \
#           (len(range(D2-(dpatch[1]/2),D2+(dpatch[1]/2)+dpatch[1]%2)) != dpatch[1]) or \
#           (len(range(D3-(dpatch[2]/2),D3+(dpatch[2]/2)+dpatch[2]%2)) != dpatch[2]):
#               
#               print('shit at '+subject_channel)
...
#        patches.shape
        ...
        for display_index in range(24,36,1):
            plt.figure(figsize=(12,8))
            plt.subplot(221)
            plt.imshow(patches[display_index,9,:,:,0], cmap='gray')
            plt.title('T1post'+str(voxelCoordinates[2][display_index-24]))
            plt.subplot(222)
...
#configFile = '/home/deeperthought/Projects/MultiPriors_MSKCC/configFiles/configFile_UNet_3D_v4_Segmenter_DGNS.py'
#workingDir = '/home/deeperthought/Projects/MultiPriors_MSKCC/'
...
#                print('Equal layer {}: {}'.format(newModel_convLayers[i].name, (model.get_layer(newModel_convLayers[i].name).get_weights()[0] == preTrained_convLayers[i].get_weights()[0]).all()))
        ...
        CV_FOLDS_ARRAYS_PATH = '/home/andy/projects/lukasSegmenter/biomedical_segmenter/CV_FOLDS_ARRAYS_PATH'
