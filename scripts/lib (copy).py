...
def extractImagePatch_parallelization(channel, subjectIndex, subject_channel_voxelCoordinates, output_shape, dpatch, percentile_normalization, preprocess_image_data=True):   
    ...
    if 'sub' in subject_channel[0]:
        t1post_channel = subject_channel[0].replace('sub','T1').replace('.nii.gz\n','_post.nii')
...
def sampleTrainData_daemon(return_dict, procnum,trainChannels,CV_FOLDS_ARRAYS_PATH, trainLabels, TPM_channel, n_patches, n_subjects, dpatch, output_classes, samplingMethod, use_coordinates, proportion_malignants_to_sample, percentile_voxel_intensity_sample_benigns, data_augmentation, proportion_to_flip, percentile_normalization, model_patch_reduction, balanced_sample_subjects=True, verbose=False, debug=False):
    ...
    pool = Pool(min(multiprocessing.cpu_count()-2,19))#mp.cpu_count() -1)
    time1 = time.time()
    ...
    return_dict[procnum] = patches, labels, all_coordinates, TPM_patches
...
def sampleTrainData(trainChannels,CV_FOLDS_ARRAYS_PATH, trainLabels, TPM_channel, n_patches, n_subjects, dpatch, output_classes, samplingMethod, use_coordinates, proportion_malignants_to_sample, percentile_voxel_intensity_sample_benigns, data_augmentation, proportion_to_flip, percentile_normalization, balanced_sample_subjects=True, verbose=False, debug=False, logfile=None):
    ...
    channel_mri = getSubjectChannels(subjectIndexes, trainChannels[0]) 
    #GET_SHAPES_INPUT = zip(channel_mri, [CV_FOLDS_SHAPES_PATH]*len(channel_mri))
    pool = Pool(multiprocessing.cpu_count()/2)#mp.cpu_count() -1)
    time1 = time.time()
    shapes = pool.map(getSubjectShapes_parallelization, channel_mri)
...
def fullSegmentation(wd, penalty_MATRIX, TPM_channel, dice_compare, dsc, smooth_dice_scores,foreground_percent_list, model, testChannels, testLabels, subjectIndex, output_classes, segmentation_dpatch, size_minibatches,output_probability, use_coordinates, percentile_normalization, model_patch_reduction, logfile, epoch, saveSegmentation = False):    
    ...
    if res[1] > 0.6:    
      ...
      target_res = res
    TPM_patches, labels, voxelCoordinates, spatial_coordinates, shape = sampleTestData(TPM_channel, testChannels, testLabels, subjectIndex, output_classes, output_dpatch, shape, use_coordinates)    
    ...
    for i in range(len(testChannels)):
        patches[:,:,:,:,i] = extractImagePatch_parallelization(testChannels[i], subjectIndex[0], voxelCoordinates, shape, segmentation_dpatch, percentile_normalization)    
...
#        my_logger('Saved segmentation of subject at: ' + output, logfile)
    ...
    if dice_compare:
      ...
      if 'BENIGN' in LABEL_CHANNEL:
        ...
        foreground_percent = np.sum(img_binary.get_data())/float(head.size)    
        ...
        try:  
          ...
          img_binary = img_binary.get_data()
          img_probs = img_probs.get_data()
          label_data = label_data[slice_of_interest, :, :]
          img_binary = img_binary[slice_of_interest,:,:]
          img_probs = img_probs[slice_of_interest,:,:]

        except:
          my_logger('Target label removed on resizing, subject: {}'.format(LABEL_CHANNEL),logfile)
          dice_compare = False
                
        score = generalized_dice_completeImages(img_binary, label_data)
        score_smooth = Generalised_dice_coef_multilabel2(label_data, img_probs)/2.
        dsc.append(score[0])
...
#######################################################################################################################################
#configFile = '/home/deeperthought/Projects/MultiPriors_MSKCC/configFiles/segmentation/Segmentation_MultiPriors_v2_trainSet.py'
...
def segment(configFile,workingDir):
    ...
    for subjectIndex in list_subjects_fullSegmentation: 
        ...
        if cfg.full_segmentation_patches:
            fullSegmentation(wd, cfg.penalty_MATRIX, cfg.TPM_channel, dice_compare, dsc, smooth_dice_scores, foreground_percent_list, model, cfg.segmentChannels, cfg.segmentLabels, subjectIndex, \
            cfg.output_classes, cfg.segmentation_dpatch, cfg.size_test_minibatches,cfg.output_probability, cfg.use_coordinates, cfg.percentile_normalization, cfg.model_patch_reduction, logfile, model_epoch, cfg.saveSegmentation)
        else:
    ...
    if dice_compare: 
      full_segm_DICE = np.mean(dsc)
      full_segm_SMOOTH_DICE = np.mean(smooth_dice_scores)
...
#configFile = '/home/deeperthought/Projects/MultiPriors_MSKCC/configFiles/configFile_MultiPriors_v2_BIG_F4_parallel_classBalanced.py'
#workingDir = '/home/deeperthought/Projects/MultiPriors_MSKCC/'
...
#                print('Equal layer {}: {}'.format(newModel_convLayers[i].name, (model.get_layer(newModel_convLayers[i].name).get_weights()[0] == preTrained_convLayers[i].get_weights()[0]).all()))
            ...
            #model.compile(loss=Generalised_dice_coef_multilabel2, optimizer=keras.optimizers.adam(lr=cfg.learning_rate), metrics=['acc', dice_coef_multilabel_bin0, dice_coef_multilabel_bin1])
            #model.compile(loss=Generalised_dice_coef_multilabel2, optimizer= keras.optimizers.adam(lr=cfg.learning_rate), metrics=['acc', dice_coef_multilabel0, dice_coef_multilabel1])
            model.summary()        
    ...
    train_data_process = Process(target=sampleTrainData_daemon, args=(return_dict, 'TRAINING', cfg.trainChannels, CV_FOLDS_ARRAYS_PATH, cfg.trainLabels,cfg.TPM_channel, cfg.n_patches, cfg.n_subjects, cfg.dpatch, cfg.output_classes, 
                                                            ...
                                                            cfg.percentile_normalization, cfg.model_patch_reduction, cfg.balanced_sample_subjects))

    val_data_process = Process(target=sampleTrainData_daemon, args=(return_dict, 'VALIDATION', cfg.validationChannels, CV_FOLDS_ARRAYS_PATH, cfg.validationLabels, cfg.TPM_channel, cfg.n_patches_val, 
                                                            ...
                                                            cfg.data_augmentation, cfg.proportion_to_flip, cfg.percentile_normalization, cfg.model_patch_reduction,
    ...
    for epoch in xrange(start_epoch,cfg.epochs):
      ...
      if epoch in cfg.epochs_for_fullSegmentation:
        ...
        for subjectIndex in list_subjects_fullSegmentation: 
            t_segment = time.time()
            if cfg.full_segmentation_patches:
                fullSegmentation(wd, cfg.penalty_MATRIX, cfg.TPM_channel, dice_compare, dsc, smooth_dice_scores, foreground_percent_list, model, cfg.testChannels, cfg.testLabels, subjectIndex, \
                cfg.output_classes, cfg.segmentation_dpatch, cfg.size_test_minibatches,cfg.output_probability, cfg.use_coordinates, cfg.percentile_normalization, cfg.model_patch_reduction, logfile, epoch, cfg.saveSegmentation)
            else:
...
#            my_logger('Reached epoch {}, changing sampling benign voxels to voxel intensity > {}'.format(epoch, cfg.percentile_voxel_intensity_sample_benigns ), logfile)
      ...
      for i in range(0, cfg.num_iter):
        ...
        val_data_process.start()
        # Validate model
        val_performance.append(train_validate_model_on_batch(model, return_dict['VALIDATION'][0],  return_dict['VALIDATION'][1], return_dict['VALIDATION'][3], return_dict['VALIDATION'][2], cfg.size_minibatches_val, history, losses,  metrics, 
        ...
        # Start next data sampling process
        train_data_process = Process(target=sampleTrainData_daemon, args=(return_dict, 'TRAINING', cfg.trainChannels, CV_FOLDS_ARRAYS_PATH, cfg.trainLabels,cfg.TPM_channel, 
...
#        image_patches = return_dict['TRAINING'][0]
#        target_labels = return_dict['TRAINING'][1]
#        TPM_patches = return_dict['TRAINING'][3] 
#        spatial_coordinates = return_dict['TRAINING'][2]
#
#        train_data_process.join()    
#        train_data_process = Process(target=sampleTrainData_daemon, args=(return_dict, 'TRAINING', cfg.trainChannels, CV_FOLDS_ARRAYS_PATH, cfg.trainLabels,cfg.TPM_channel, 
#                                                                          cfg.n_patches, cfg.n_subjects, cfg.dpatch, cfg.output_classes, 
...
#        train_data_process.start()
#        
#        image_patches = np.concatenate([image_patches, return_dict['TRAINING'][0]])
#        target_labels = np.concatenate([target_labels, return_dict['TRAINING'][1]])
#        TPM_patches = np.concatenate([TPM_patches, return_dict['TRAINING'][3]])
#        spatial_coordinates = np.concatenate([spatial_coordinates, return_dict['TRAINING'][2]])     
        
        
        # Train model
        epoch_loss, epoch_metrics = train_validate_model_on_batch(model,return_dict['TRAINING'][0],return_dict['TRAINING'][1],return_dict['TRAINING'][3] , return_dict['TRAINING'][2],cfg.size_minibatches,history,losses,metrics,cfg.output_classes, logfile)  
        try:
